...
> 状态：**maintained UI reference**。V1 视觉与组件设计保留；当前 2–4 人、Agent 和 attempt 架构以 [architecture.md](architecture.md) 为准。移动端 placing 的“禁止滚动”旧结论已被 [2026-07-10 mobile viewport 方案](../plans/2026-07-10-mobile-gameplay-viewport-fixes.md) 替代。
...
- **聊天消息结构化**：`Message` 增加 `attemptId`，按座位归属显示发送者，`kind==='agent'` 使用 Agent 样式。前端只展示当前 attempt 的消息；团队策略由服务端 `StrategyCompiler` 生成，前端不直接调用模型。
...
- **移动端 placing 当前口径**：允许页面纵向滚动，保证钟面、状态条和手牌均可达；具体滚动容器、横竖屏旋转和安全区规则以 [2026-07-10 mobile viewport 方案](../plans/2026-07-10-mobile-gameplay-viewport-fixes.md) 为准。
...
后续状态：M8 的固定 4 座、多人 selector、多人队友栏和 Agent 座位 UI 已基本落地；待补 3/4 人 E2E。M9 前端后续需增加团队策略只读摘要、Agent fallback/不可用状态和可选的对局复盘入口。
...
> M8 的固定 4 座、2–4 人弹性开局、dealRules 可见性和 Agent 加/撤入口已基本落地，不再属于“仅预留”。M9 的真实模型、团队策略和 attempt memory 仍由服务端实现；前端只展示 Agent 座位、消息、思考/降级状态和结构化复盘数据。
