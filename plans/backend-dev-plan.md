...
**v1.1 可扩展（N 人 + 机器人）**：MVP **只交付并测试 2 人真人**，但数据模型与核心逻辑从一开始就**与玩家数无关、与行动者无关**——① 座位从固定 A/B 改为 **N 座位**（capacity 2/3/4）；② **发牌规则表**按玩家数分流（2:每人6张中4可见端2盲+打满2张翻牌；3:每人4张全可见；4:每人3张全可见；deck 恒 12 张，盲牌/翻牌机制仅 2 人有）；③ 回合从 A↔B 交替泛化为**抢先手 + 座位序循环**；④ 引入**动作层**（`actions.ts`），人类 socket 事件与未来机器人决策走**同一内部入口**；⑤ **预留异步 `PlayerAgent` 接口**（未来 LLM-agent：API 调用 + 上下文 + memory），MVP 不实现具体 bot，接口与驱动位就位，后续扩展工作量小。
  ...
  server/                 # Express + Socket.IO（TypeScript）
    src/
      ...
      agent/
        ...
        agentDriver.ts    # 预留：轮到 bot 座位时驱动其经动作层行动（MVP 不实现具体 bot）
      levels/
...
- `Seat { id, kind: 'human'|'bot', nick, agentId?, connected }`；`ready` / `hands` / `playedCount` 均**按座位 id 索引**（不再是 `{A,B}`）。`host` = 某座位 id（第一个准备者）。
...
- **聊天消息结构化**：`chat: Message[]`，`Message { id, senderSeatId, kind: 'human'|'bot', text, ts }`。座位归属让前端按座位渲染、让 agent 区分发言者；`kind` 区分人/机消息。`chat` 进入 `room:state` 广播，同时是 §2.10 `AgentRoomView` 的来源字段（讨论聊天对全体玩家可见，喂给 agent 作策略上下文）。`senderSeatId`/`kind`/`ts` 一律由**服务端**赋值，不信任客户端自填发送者。
...
### 2.5 权威计时器（`timers.ts`）
服务端持有真实 deadline，`timer:sync` 周期下发剩余时间：
...
- **预留 `PlayerAgent`（异步接口）**：
  ...
  异步签名为未来 **LLM-agent**（API 调用 + 上下文 + memory）预留；MVP 不实现具体 bot。
- **`AgentRoomView`（bot 视角输入）**= **§2.4 遮蔽后的对局状态** + **该局完整讨论聊天记录（`chat`，按座位归属，见 §2.1 `Message`）** + **当前阶段上下文**：
  - 讨论 `chat` transcript 是 agent 的**主要策略输入**（对应人类「讨论默契」），出牌 / 提示 / 发言决策时都按完整 transcript 提供。
  - 牌值仍走 §2.4 遮蔽：bot 不能看它看不到的牌（对己暗牌 / 桌面暗牌 / 队友手牌）；但**讨论聊天对全体玩家可见**，故完整下放，不遮蔽。
- **预留 `agentDriver`**：
  - **出牌 / 提示阶段**：`turn` 落到 `kind==='bot'` 座位时，驱动其调用 `PlayerAgent` 并把结果经动作层落子；决策须在**思考时间窗口内**返回，**绝不让 bot 触发超时判负**（超时兜底用合法随机落子）。提示窗口同理。
  - **讨论阶段**：在讨论时间窗口内可调用 `PlayerAgent.decideDiscussion`，把返回消息经**服务端内部入口**注入 `chat`（`kind='bot'`、带 `senderSeatId`），随 `room:state` 广播——**bot 不走 `chat:send` socket 入站**。沿用「窗口内返回、超时不阻塞其他玩家」约束。
- **MVP 不实现任何具体 bot**：`PlayerAgent`（含 `decideDiscussion`）仅有接口与类型，`agentDriver` 留桩；动作层与 turnOrder 按 N 人 + 行动者无关写好。未来接 LLM-agent 时只新增 agent 实现，**不动核心游戏逻辑**。
...
- **预留（M9）**：`host:addBot { seatId, agentId }` / `host:removeBot { seatId }` 给空座位加/撤机器人；MVP 不实现。出站事件不变：bot **不收 `player:hand`**、**不通过 `chat:send` 入站**——其发言（`kind='bot'`）与落子均由 `agentDriver` 经内部入口/动作层注入后随 `room:state` 广播（`agentDriver` 内部按遮蔽视角取状态）。
...
- **主动离开**（`player:leave`）：立即释放座位（不等保留期）。
...
- **行动者无关测试**：直接调 `actions.applyPlacement`（绕过 socket，模拟未来 bot 驱动）与经 socket 的人类落子，对相同状态产生**一致结果**——验证动作层不耦合“人/机”。
- **`AgentRoomView` 测试**（预留断言，MVP 不跑 bot）：构造 bot 视角的 `AgentRoomView`，断言**含完整讨论 chat**（全可见），但**不含该 bot 视角外的牌值**（对己暗牌 / 桌面未翻牌 / 队友手牌）——chat 全给、牌值仍遮蔽。
- 对齐设计方案 §测试计划逐条断言。
...
3. **M2 入座 / 准备 / 房主 + payload 校验**：`validation/schemas.ts`（zod）接入所有 handler；`player:join` 入座与房间满拒绝、同昵称重连恢复座位/host（按 §3.1）；`player:leave` 立即释放；`player:ready` + 第一个准备者记 host；`room:state` 广播 + `room:error`。
...
8. **M7 打磨 + Railway 验收**：`room:reset`、断线 60s 座位保留 + host 离开转移/软重置（§3.1）、计时器幂等回归、可见性防泄漏与 payload 校验测试、集成测试全闭环；**Railway 端到端验收**——挂 Volume 部署，验证 PORT/healthcheck、WebSocket 实时同步、重启后进行中对局清空但 `progress.json` 保留、`SIGTERM` 关停前成功 flush 写盘。
...
10. **M9 LLM-agent 机器人**：实现 `PlayerAgent`（含 `decidePlacement`/`decideHint`/**`decideDiscussion` 讨论发言**；API 调用 + 上下文 + memory）与 `agentDriver`（出牌/提示思考窗口内决策、超时合法兜底；**讨论窗口内注入 bot 发言**）；agent 输入 `AgentRoomView` 含**完整讨论 transcript**（§2.10）；大厅支持房主给空座位加 bot（如 2 真人 + 2 bot）；bot 视角牌值走 §2.4 遮蔽。核心动作层 / turnOrder 不变。
