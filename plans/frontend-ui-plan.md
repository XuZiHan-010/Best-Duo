...
全程常驻 **顶部状态条 `<TopBar>`**：品牌（`◷ CARD GAME`）、当前关号、阶段名、**按座位数渲染的玩家状态点**（`seats[]` 派生，2 人即两点；bot 座位带 bot 标记）、（局中）提示标记余额、连接状态。回合指示泛化为「N 人中轮到谁」（局中高亮当前 `turn` 座位）。
...
- **bot 座位（为 M9 预留）**：座位 `kind==='bot'` 时显示 **bot 徽标**（替代昵称），无「准备」交互（bot 由服务端视为已就绪）；MVP 不出现 bot 座位。
- 设置控件：非房主渲染为 `disabled` + `aria-disabled`，并提示「仅房主可调整」。
- **设置面板预留两项扩展入口**：① **人数 `capacity`（2/3/4）** 分段单选；② **房主给空座位加/撤 bot** 的按钮。二者在 **MVP 锁定**——`capacity` 固定 2、加 bot 入口 `disabled`，仅作接口占位（对齐后端「MVP 锁 2、不实现 bot」，M8 放开 3/4、M9 接 bot）。
...
- **聊天消息结构化（chat→agent 前端侧）**：按 `@take-time/shared` 的 `Message { id, senderSeatId, kind: 'human'|'bot', text, ts }` 渲染——**按座位归属显示发送者**，`kind==='bot'` 的消息带 bot 样式标识。讨论聊天是**全体玩家（含未来 agent）共享的策略上下文**；前端只负责消息结构化、座位归属、进入共享 `room:state`，**由后端据此喂 [`AgentRoomView`](backend-dev-plan.md)**，前端不直接对接 agent。bot 发言由服务端注入后随广播到达，前端按 `kind` 渲染即可（MVP 不出现 bot 消息）。
...
| 桌上暗置牌 | 区段内暗置牌 = 铜框磨砂长方形（无数字）；**每段恒显「N 张」计数** |
...
- **bot 座位（为 M9 预留）**：轮到 `kind==='bot'` 座位时，右栏回合状态显示「（bot 名）思考中…」指示，**不渲染手牌交互**（bot 落子由服务端注入，前端只随 `room:state` 更新桌面暗牌堆叠）；MVP 不出现 bot。回合归属判断按 `turn` 座位 id，对 N 人通用。
...
- **倒计时**：以服务端 `timer:sync` 下发的 deadline 为准，每次 `timer:sync` 重新校准，前端用 `requestAnimationFrame` 平滑插值显示剩余秒，但**判负由服务端宣布**（前端归零只做视觉，不发判负）。前端倒计时到 0 后只显示「等待结果…」，**不本地切到 result**，结果以 `room:state` / `game:result` 回包为准。（不做 NTP 式时钟偏移估算——周期性 `timer:sync` 重发 deadline 已足够自校正。）
...
> 本计划中所有 **N 座位渲染、bot 座位表现、`capacity` 选择、加/撤 bot 入口、dealRules 驱动可见性、`kind='bot'` 聊天渲染** 项,均为 **M8（3/4 人）/ M9（LLM-agent）扩展预留**。**v1 MVP 锁 2 人真人、不实现任何 bot**：`capacity` 固定 2、加 bot 入口 `disabled`、不会出现 bot 座位/消息。预留的目的是让 store（座位数组化 §4.2）、UI（座位/回合/手牌均经 seats[]/掩码派生）与聊天模型（结构化 `Message`）在 M8/M9 扩展时**不需重写**。与后端 [backend-dev-plan §2.9/§2.10/§6](backend-dev-plan.md) 的 N 人 + 行动者无关架构一一对齐。
...
随关卡难度递增，背景与强调色可做阶段性变化，增强沉浸感：
- **★★（基础）**：现有深蓝 `--bg-1 #0c1320` + 黄铜 `--brass`（默认主题）。
- **★★★（中等）**：背景偏深紫蓝，强调色转玫瑰金 `~#C9758A`。
- **★★★★（困难）**：更深的午夜近黑，强调色转冷银 `~#8AA4C0`。
- **★★★★★（极难）**：纯黑底，强调色转血红/炭灰，倒计时默认即危险态色调。
- 实现方式：关卡加载时在 `<html>` 上附加 `data-difficulty="★★★"` class，各 Token 通过 `:root[data-difficulty="★★★"]` 覆写；过渡用 `transition: background-color 1s ease`。
- **此项为 M7+ Future Work，MVP 阶段所有关卡统一使用默认主题。**
