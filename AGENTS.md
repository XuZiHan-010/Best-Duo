...
> 本文件是给 AI agent 的项目导航目录。先读这里，了解项目在做什么、相关文件在哪、约定有哪些。
...
- 手牌可见性与桌面暗牌的牌值由服务端按规则遮蔽后再下发；桌面暗置牌可以公开颜色，但不公开具体数值。
...
- 卡牌带 `value`（数值）和 `color`（`'white' | 'black'`）两个维度；桌面暗置阶段显示颜色、遮蔽数值，揭示时公开数值。

## 待补全项（影响实现，需用户提供）

- 关卡内容：**总关卡数 N（如 40）**；前 10 关已设计于 [levels/](levels/)，后续关卡持续补充。
