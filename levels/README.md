...
> 关卡的数据结构见 [设计方案 · 关卡与挑战模型](../docs/take-time-web-prototype.md)。运行时哪些关已通关由服务端 `progress.clearedLevels` 记录并持久化；本文件夹只描述关卡“设计内容”。
...
| `min-color-cards { segment, color, count }` | 某区段至少包含 count 张指定颜色的牌（`color: 'white' | 'black'`） |
| `all-distinct { segment }` | 某区段内各牌数值互不相同（不限张数） |
| `has-duplicate-value { segment }` | 某区段内至少存在两张数值相同的牌（不限张数） |
...
关卡仍在陆续设计，规则会持续更新。为保证「改规则不需大改」，每条规则只走**一条固定链路**，新增一个条件类型只需碰 5 处、各加一小段：

1. **本 README 词汇表**：加一行 `type` 与含义（人类口径）。
2. **`shared/src/level.ts`**：把新 `type` 加进 `Condition` 联合类型（前后端共用，单一来源）。
3. **`server` 条件引擎 `conditionEngine.ts`**：加一个 `case`，输入 6 段总和/张数 → `{ pass, message }`。若条件依赖**牌面颜色或出牌顺序**（如 `segment-colors`、`min-color-cards`、`placement-order`），改用传入的 `placements`（含 `color`、`placedAt`）而非仅总和。
4. **`server/src/game/solver.ts`**：让发牌前可解性校验也理解新条件，避免抽到运行时无法保证可解的牌面。
5. **`client` `lib/conditionText.ts`**：加一条「条件 → 中文」映射；若该条件绑定单个区段，也同步考虑 `segmentHints.ts` 的钟面徽标。
...
| [第 5 关](level-05.md) | ★ | 区3 偶数 + 区5 总和 16–20 + 区6 至少一对同值 | 已设计 |
| [第 6 关](level-06.md) | ★ | 区2 总和 9 + 区4 恰好 2 张 + 区6 恰好 3 张且至少一黑一白 | 已设计 |
| [第 7 关](level-07.md) | ★★ | 区3 恰好一黑一白 + 区5 奇数 + 区6 总和 22–24 | 已设计 |
| [第 8 关](level-08.md) | ★★ | 区2 总和 8–12 + 区4/区5 总和差 ≤ 3 + 区6 至少 2 张白牌 | 已设计 |
| [第 9 关](level-09.md) | ★★ | 区1 总和 6 + 区6 恰好 3 张、数值不同、2 黑 1 白 | 已设计 |
| [第 10 关](level-10.md) | ★★ | 区2 总和 10 + 区3 偶数 + 区4 一黑一白 + 区5/区6 总和差 ≤ 4 | 已设计 |
| … | | 后续关卡待补充 | 待补 |
